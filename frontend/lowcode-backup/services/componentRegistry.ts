...
export interface ComponentDefinition {
  ...
  description?: string;
  component?: any;
  asyncLoader?: () => Promise<any>; // 异步加载函数
  props?: Array<{
    name: string;
    type: string;
    default?: any;
...
class ComponentRegistryImpl implements ComponentRegistry {
  ...
  async preloadComponents(types: string[]): Promise<void> {
    const preloadItems = types
      .map(type => this.components[type])
      .filter(component => component && component.asyncLoader)
      .map(component => ({
        type: component!.type,
...
export default {
  ...
  preloadComponent: safePreloadComponent,
