我需要查看MongoDB仓储相关的完整接口和实现，以分析它们之间的调用关系。让我先查看MongoDB仓储的接口定义和工厂接口。
...
filePath: f:\ProjectCode\OrlanFackeMicro\src\FakeMicro.DatabaseAccess\Interfaces\Mongo\IMongoRepository.cs
f:\ProjectCode\OrlanFackeMicro\src\FakeMicro.DatabaseAccess\Interfaces\Mongo\IMongoRepositoryFactory.cs
          
文件路径不正确，让我搜索一下MongoDB相关接口的正确位置。
...
query: IMongoRepository|IMongoRepositoryFactory
          
现在我找到了MongoDB仓储相关接口和实现的正确位置，让我查看这些核心文件的完整内容来分析调用关系。
...
filePath: f:\ProjectCode\OrlanFackeMicro\src\FakeMicro.DatabaseAccess\Interfaces\IMongoRepository.cs
f:\ProjectCode\OrlanFackeMicro\src\FakeMicro.DatabaseAccess\Interfaces\IMongoRepositoryFactory.cs
f:\ProjectCode\OrlanFackeMicro\src\FakeMicro.DatabaseAccess\Interfaces\IRepositoryFactory.cs
          
让我查看MongoRepositoryFactory的完整实现，以及DynamicRepositoryFactory的实现，来了解MongoDB仓储的调用关系。
...
filePath: f:\ProjectCode\OrlanFackeMicro\src\FakeMicro.DatabaseAccess\MongoRepositoryFactory.cs
f:\ProjectCode\OrlanFackeMicro\src\FakeMicro.DatabaseAccess\DynamicRepositoryFactory.cs
          
让我查看DynamicRepositoryFactory的完整实现，以及依赖注入配置文件，了解MongoDB仓储的调用关系。
...
f:\ProjectCode\OrlanFackeMicro\src\FakeMicro.DatabaseAccess\MongoDBServiceCollectionExtensions.cs
f:\ProjectCode\OrlanFackeMicro\src\FakeMicro.DatabaseAccess\DatabaseServiceExtensions.cs
          
让我查看项目中使用MongoDB仓储的示例，比如在Grains或API项目中的具体调用方式。
...
query: IMongoRepository|CreateMongoRepository|GetPagedAsync
