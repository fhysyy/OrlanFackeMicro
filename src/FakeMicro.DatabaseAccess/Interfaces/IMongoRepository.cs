using FakeMicro.Interfaces;using FakeMicro.Interfaces.Models;
using MongoDB.Bson;
...
public interface IMongoRepository<TEntity, TKey> : IRepository<TEntity, TKey> where TEntity : class
{
    ...
    #region MongoDB特有的CRUD方法（带databaseName和collectionName参数重载）
...
/// <typeparam name="TEntity">实体类型</typeparam>
/// <typeparam name="TKey">主键类型</typeparam>
public interface IMongoReadRepository<TEntity, TKey> : IReadRepository<TEntity, TKey>, IBaseRepository<TEntity, TKey> where TEntity : class
{
    /// <summary>
    /// 获取MongoDB集合
    /// </summary>
    ...
    /// <summary>
    /// 使用FilterDefinition查询文档（支持指定数据库）
    /// </summary>
    Task<List<TEntity>> FindAsync(FilterDefinition<TEntity> filter, string? databaseName, CancellationToken cancellationToken = default);

    /// <summary>
    /// 使用FilterDefinition查询文档（支持指定数据库和集合）
    /// </summary>
    Task<List<TEntity>> FindAsync(FilterDefinition<TEntity> filter, string? databaseName, string? collectionName, CancellationToken cancellationToken = default);
}
...
/// <typeparam name="TEntity">实体类型</typeparam>
/// <typeparam name="TKey">主键类型</typeparam>
public interface IMongoWriteRepository<TEntity, TKey> : IWriteRepository<TEntity, TKey>, IBaseRepository<TEntity, TKey> where TEntity : class
{
    /// <summary>
    /// 添加实体（支持指定数据库）
    /// </summary>
    Task<TEntity> AddAsync(TEntity entity, string? databaseName, CancellationToken cancellationToken = default);

    /// <summary>
    /// 添加实体（支持指定数据库和集合）
    /// </summary>
    Task<TEntity> AddAsync(TEntity entity, string? databaseName, string? collectionName, CancellationToken cancellationToken = default);

    /// <summary>
    /// 更新实体（支持指定数据库）
    /// </summary>
    Task<bool> UpdateAsync(TEntity entity, string? databaseName, CancellationToken cancellationToken = default);

    /// <summary>
    /// 更新实体（支持指定数据库和集合）
    /// </summary>
    Task<bool> UpdateAsync(TEntity entity, string? databaseName, string? collectionName, CancellationToken cancellationToken = default);

    /// <summary>
    /// 根据主键删除实体（支持指定数据库）
    /// </summary>
    Task<bool> DeleteByIdAsync(TKey id, string? databaseName, CancellationToken cancellationToken = default);

    /// <summary>
    /// 根据主键删除实体（支持指定数据库和集合）
    /// </summary>
    Task<bool> DeleteByIdAsync(TKey id, string? databaseName, string? collectionName, CancellationToken cancellationToken = default);
