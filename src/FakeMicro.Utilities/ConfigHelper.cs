...
namespace FakeMicro.Utilities
{
    ...
    public static class ConfigHelper
    {
        ...
        /// 从环境变量获取配置值（带类型转换）
        /// </summary>
        public static T? GetEnvironmentVariable<T>(string variableName, T? defaultValue = default)
        {
        ...
        {
            return GetEnvironmentVariable($"ConnectionStrings__{name}") 
                ?? GetEnvironmentVariable($"CONNECTIONSTRINGS_{name.ToUpperInvariant()}")
                ?? defaultValue 
                ?? string.Empty;
        }
        ...
        {
            return GetEnvironmentVariable($"AppSettings__{key}")
                ?? GetEnvironmentVariable($"APPSETTINGS_{key.ToUpperInvariant()}")
                ?? defaultValue
                ?? string.Empty;
        }
        ...
        /// 环境变量配置提供者
        /// </summary>
        public class EnvironmentConfigurationProvider<TConfigType> : ConfigurationProvider<TConfigType> where TConfigType : class, new()
        {
            private readonly string _prefix;

            public EnvironmentConfigurationProvider(string prefix = "")
            {
                _prefix = prefix;
                Reload();
            }
            ...
            public override void Reload()
            {
                _configuration = LoadFromEnvironmentVariables(_prefix);
            }
            ...
            public override void Save()
            {
                throw new NotSupportedException("Environment variables cannot be saved through this provider");
            }
            ...
            private TConfigType LoadFromEnvironmentVariables(string prefix)
            {
                ...
                foreach (var property in properties)
                {
                    ...
                    {
                        var envVarName = string.IsNullOrEmpty(prefix) 
                            ? property.Name 
                            : $"{prefix}_{property.Name}";

                        var value = GetEnvironmentVariable(envVarName);
                        if (!string.IsNullOrEmpty(value))
                        {
                            ...
                            {
                                LoggerHelper.Error($"Failed to set property {property.Name} from environment variable {envVarName}", ex);
                            }
                        }
                    }
                }
